...
* **Suitability for purpose**: The CDM aims to provide data organized in a way optimal for analysis, rather than the operational needs of health care providers or payers. 
* **Data protection**: All data that might jeopardize the identity and protection of patients, such as names, birthdays, addresses etc. are limited. Exceptions are possible where the research expressly requires more detailed information, such as more precise birth dates for the study of infants.
* **Design of Domains**: Clinical Events are organized in Domains (see chapter \@ref(StandardizedVocabularies)), which are modeled in a person-centric relational data model with a separate table for each Domain. Domain tables all share Foreign Keys to the PERSON table for identifying patients and a date, and Domain-specific attributes only applicable to the Domain (e.g. DAYS_SUPPLY for the Drug Domain). All other data can be preserved as an observation in the observation table in an entity-attribute-value structure.
* **Standardized Vocabularies**: The content of all Domain table records is normalized through Concepts, which are common to all OMOP CDM instances the reference tables of the Standardized Vocabularies. With few exception, there is no verbatim content in the standardized content of these tables. 
...
* **Technology neutrality**: The CDM does not require a specific technology. It can be realized in any relational database, such as Oracle, SQL Server etc., or non-RDBS technology such as Hadoop or SAS.
...
* **Backwards compatibility**: Changes between CDM versions are made with backwards compatibility in mind, such that little information is lost that was present previously. This rule is not always possible to keep 100%, though. All versions are stored in the github repository [(https://github.com/OHDSI/CommonDataModel)](https://github.com/OHDSI/CommonDataModel). 
