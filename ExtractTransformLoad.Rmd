...
Often a mapping is not a one-time effort. As the data source is updated, perhaps new source codes are added; as the vocabulary is updated, perhaps new mappings are required. 
...
The ETL process is difficult to master for many reasons, not the least of which is the fact that we are all processing unique source data, making it hard to create a "one-size-fits-all" solution. However, over the years we have learned the following hard-won lessons.
