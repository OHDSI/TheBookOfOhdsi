...
\begin{verbatim}
## [1] "SELECT * FROM temp_schema.xvkvs3hqchildren ;"
\end{verbatim}
...
\emph{Contributors: Kristin Kostka, Patrick Ryan, Jon Duke, Juan Banda, Joel Swerdel, Elle Palmer}
...
The systematic reuse of cohort definitions and the subsequent evaluation of phenotypes to characterize components of disease remains an ongoing piece of work within the OHDSI Community \citep{Swerdel2019phevaluator}. A literature review of over 33 studies found significant heterogeneity in phenotype algorithms used, validation methods, and results (\citep{Rubbo2015phenotypes}). In general, the validation of a rules-based cohort definition or probabilistic algorithm can be thought of as a test of the proposed cohort compared to some form of ``gold standard'' reference (e.g.~manual chart review of cases).
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/Cohorts/CohortPractice} \caption{Logical Diagram of Intended Cohort}\label{fig:CohortPractice}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{images/Cohorts/ATLAS-cohort.png}
\caption{\label{fig:ATLAScohort}Navigating to ATLAS Cohort Definition module}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{images/Cohorts/ATLAS-newcohort.png}
\caption{\label{fig:ATLASnewcohort}Navigating to ATLAS New Cohort button}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{images/Cohorts/ATLAS-defineacohort.png}
\caption{\label{fig:ATLASdefineacohort}New Cohort Definition}
\end{figure}

Before you do anything else, you will need to change the name of the cohort from \emph{New Cohort Definition} to your own unique name for this cohort. You may opt for a name like \emph{New users of ACE inhibitors as first-line monotherapy for hypertension}.
...
\begin{figure}
\centering
\includegraphics{images/Cohorts/ATLAS-initialevent.png}
\caption{\label{fig:ATLASinitialevent}Adding an Initial Event}
\end{figure}

ATLAS provides descriptions below each criteria to help you. If we were building a CONDITION\_OCCURRENCE based criteria, our question would be looking for patients with a specific diagnosis. If we were building a DRUG\_EXPOSURE based criteria, our question would be looking for patients with a specific drug or drug class. Since we want to find \emph{patients who initiate ACE inhibitors monotherapy as first-line treatments for hypertension}, we want to choose a DRUG\_EXPOSURE criteria. You may say, \emph{but we also care about hypertension as a diagnosis}. You are correct. Hypertension is another criteria we will build. However, eligibility to enter this cohort is predicated on being a new-user of ACE inhibitors as monotherapy. The diagnosis of hypertension is what we call an \emph{additional qualifying criteria}. We will return to this once we build this criteria. We will click to add a DRUG\_EXPOSURE criteria.

\begin{figure}
\centering
\includegraphics{images/Cohorts/ATLAS-drugexposure.png}
\caption{\label{fig:ATLASdrugexposure}Building a Drug Exposure Criteria}
\end{figure}

The screen will update with your selected criteria but you are not done yet. You now need to tell ATLAS which concept set you want to associate with this DRUG\_EXPOSURE criteria. You will need to click the down arrow to open the dialogue box to import a concept set.
\includegraphics{images/Cohorts/ATLAS-importaconcept.png}

\textbf{Scenario 1: You have not built a concept set.} Imagine you've got a blank slate here. You have not assembled your concept sets to retrieve to apply to your criteria. At a very basic level, this will require you to utilize the \emph{Search} function to find and choose the concepts you would like to specify. You can refer back to (see Chapter \ref{StandardVocabularies}) on how to navigate the OMOP vocabularies to find clinical concepts of interest. This chapter will not focus on the assembly of concept sets. You may utilize the OHDSI Resources page to find bite sized ATLAS tutorials that can help you understand how to search and build a concept set.

\textbf{Scenario 2: You've already built a concept set.} If you've already created a concept set and saved it in ATLAS, you can click to \emph{`Import a Concept Set'} where you will be then prompted to find your concept in the concept set repository of your ATLAS.
\includegraphics{images/Cohorts/ATLAS-findingyourconcept.png}

In the above example, the user typed in the name given to this concept set \emph{`ace inhibitors'} in the right hand search. This shortened the concept set list to only concepts with matching names. From there, the user can click on the row of the concept set to select it. (Note: the dialogue box will disappear once you have selected a concept set.) You are not done yet. Your question is looking for new users or the first time in someone's history they are exposed to ACE inhibitors. You need to click to \emph{`Add attribute'}.

\begin{figure}
\centering
\includegraphics{images/Cohorts/ATLAS-firstexposure.png}
\caption{\label{fig:ATLASfirstexposure}Adding Attribute to Initial Criteria - Step 1}
\end{figure}

You will want to select the \emph{First Exposure Criteria}. From there, the window will automatically close. Notice, you could specify other attributes of a criteria you build. You could specify an attribute of age at occurrence, the date of occurrence, gender or other attributes related to the drug. Criteria available for selection will look different for each domain.

\begin{figure}
\centering
\includegraphics{images/Cohorts/ATLAS-firsttimeever.png}
\caption{\label{fig:ATLASfirsttimeever}Adding Attribute to Initial Criteria - Step 2}
\end{figure}
...
\includegraphics{images/Cohorts/ATLAS-IC1.png}

You will then want to add another criteria to look for patients: \emph{with exactly 0 occurrences of hypertension drugs all days before and 1 day before index start date (no exposure to HT drugs before an ACE inhibitor)}. If you're following along, this would come out looking like this:
\includegraphics{images/Cohorts/ATLAS-IC2.png}
...
If you'd prefer to query a database directly, you may write a SQL query to directly look for your particular cohort. OHDSI does not require a specific data layer. Where possible, you shoudl utilize \href{https://github.com/OHDSI/SqlRender}{SQL Render} to render parameterized SQL, and translating it to different SQL dialects so you can share your cohort definition across network sites. Below is an example of how to write a query directly to a database for your cohort:

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{CREATE} \KeywordTok{TABLE}\NormalTok{ #Codesets (}
\NormalTok{  codeset_id }\DataTypeTok{int} \KeywordTok{NOT} \KeywordTok{NULL}\NormalTok{,}
\NormalTok{  concept_id bigint }\KeywordTok{NOT} \KeywordTok{NULL}
\NormalTok{)}
\NormalTok{;}

\KeywordTok{INSERT} \KeywordTok{INTO}\NormalTok{ #Codesets (codeset_id, concept_id)}
\KeywordTok{SELECT} \DecValTok{0} \KeywordTok{as}\NormalTok{ codeset_id, c.concept_id }\KeywordTok{FROM}\NormalTok{ (}\KeywordTok{select} \KeywordTok{distinct}\NormalTok{ I.concept_id }\KeywordTok{FROM}
\NormalTok{( }
  \KeywordTok{select}\NormalTok{ concept_id }\KeywordTok{from}\NormalTok{ @vocabulary_database_schema.CONCEPT }\KeywordTok{where}\NormalTok{ concept_id }\KeywordTok{in}\NormalTok{ (}\DecValTok{316866}\NormalTok{)}
\KeywordTok{UNION}  \KeywordTok{select}\NormalTok{ c.concept_id}
  \KeywordTok{from}\NormalTok{ @vocabulary_database_schema.CONCEPT c}
  \KeywordTok{join}\NormalTok{ @vocabulary_database_schema.CONCEPT_ANCESTOR ca }\KeywordTok{on}\NormalTok{ c.concept_id }\OperatorTok{=}\NormalTok{ ca.descendant_concept_id}
  \KeywordTok{and}\NormalTok{ ca.ancestor_concept_id }\KeywordTok{in}\NormalTok{ (}\DecValTok{316866}\NormalTok{)}
  \KeywordTok{and}\NormalTok{ c.invalid_reason }\KeywordTok{is} \KeywordTok{null}

\NormalTok{) I}
\NormalTok{) C;}
\KeywordTok{INSERT} \KeywordTok{INTO}\NormalTok{ #Codesets (codeset_id, concept_id)}
\KeywordTok{SELECT} \DecValTok{2} \KeywordTok{as}\NormalTok{ codeset_id, c.concept_id }\KeywordTok{FROM}\NormalTok{ (}\KeywordTok{select} \KeywordTok{distinct}\NormalTok{ I.concept_id }\KeywordTok{FROM}
\NormalTok{( }
  \KeywordTok{select}\NormalTok{ concept_id }\KeywordTok{from}\NormalTok{ @vocabulary_database_schema.CONCEPT }\KeywordTok{where}\NormalTok{ concept_id }\KeywordTok{in}\NormalTok{ (}\DecValTok{1319998}\NormalTok{,}\DecValTok{1317967}\NormalTok{,}\DecValTok{991382}\NormalTok{,}\DecValTok{1332418}\NormalTok{,}\DecValTok{1314002}\NormalTok{,}\DecValTok{40235485}\NormalTok{,}\DecValTok{1335471}\NormalTok{,}\DecValTok{1322081}\NormalTok{,}\DecValTok{1338005}\NormalTok{,}\DecValTok{932745}\NormalTok{,}\DecValTok{1351557}\NormalTok{,}\DecValTok{1340128}\NormalTok{,}\DecValTok{1346823}\NormalTok{,}\DecValTok{1395058}\NormalTok{,}\DecValTok{1398937}\NormalTok{,}\DecValTok{1328165}\NormalTok{,}\DecValTok{1363053}\NormalTok{,}\DecValTok{1341927}\NormalTok{,}\DecValTok{1309799}\NormalTok{,}\DecValTok{1346686}\NormalTok{,}\DecValTok{1353776}\NormalTok{,}\DecValTok{1363749}\NormalTok{,}\DecValTok{956874}\NormalTok{,}\DecValTok{1344965}\NormalTok{,}\DecValTok{1373928}\NormalTok{,}\DecValTok{974166}\NormalTok{,}\DecValTok{978555}\NormalTok{,}\DecValTok{1347384}\NormalTok{,}\DecValTok{1326012}\NormalTok{,}\DecValTok{1386957}\NormalTok{,}\DecValTok{1308216}\NormalTok{,}\DecValTok{1367500}\NormalTok{,}\DecValTok{1305447}\NormalTok{,}\DecValTok{907013}\NormalTok{,}\DecValTok{1307046}\NormalTok{,}\DecValTok{1309068}\NormalTok{,}\DecValTok{1310756}\NormalTok{,}\DecValTok{1313200}\NormalTok{,}\DecValTok{1314577}\NormalTok{,}\DecValTok{1318137}\NormalTok{,}\DecValTok{1318853}\NormalTok{,}\DecValTok{1319880}\NormalTok{,}\DecValTok{40226742}\NormalTok{,}\DecValTok{1327978}\NormalTok{,}\DecValTok{1373225}\NormalTok{,}\DecValTok{1345858}\NormalTok{,}\DecValTok{1350489}\NormalTok{,}\DecValTok{1353766}\NormalTok{,}\DecValTok{1331235}\NormalTok{,}\DecValTok{1334456}\NormalTok{,}\DecValTok{970250}\NormalTok{,}\DecValTok{1317640}\NormalTok{,}\DecValTok{1341238}\NormalTok{,}\DecValTok{942350}\NormalTok{,}\DecValTok{1342439}\NormalTok{,}\DecValTok{904542}\NormalTok{,}\DecValTok{1308842}\NormalTok{,}\DecValTok{1307863}\NormalTok{)}
\KeywordTok{UNION}  \KeywordTok{select}\NormalTok{ c.concept_id}
  \KeywordTok{from}\NormalTok{ @vocabulary_database_schema.CONCEPT c}
  \KeywordTok{join}\NormalTok{ @vocabulary_database_schema.CONCEPT_ANCESTOR ca }\KeywordTok{on}\NormalTok{ c.concept_id }\OperatorTok{=}\NormalTok{ ca.descendant_concept_id}
  \KeywordTok{and}\NormalTok{ ca.ancestor_concept_id }\KeywordTok{in}\NormalTok{ (}\DecValTok{1319998}\NormalTok{,}\DecValTok{1317967}\NormalTok{,}\DecValTok{991382}\NormalTok{,}\DecValTok{1332418}\NormalTok{,}\DecValTok{1314002}\NormalTok{,}\DecValTok{40235485}\NormalTok{,}\DecValTok{1335471}\NormalTok{,}\DecValTok{1322081}\NormalTok{,}\DecValTok{1338005}\NormalTok{,}\DecValTok{932745}\NormalTok{,}\DecValTok{1351557}\NormalTok{,}\DecValTok{1340128}\NormalTok{,}\DecValTok{1346823}\NormalTok{,}\DecValTok{1395058}\NormalTok{,}\DecValTok{1398937}\NormalTok{,}\DecValTok{1328165}\NormalTok{,}\DecValTok{1363053}\NormalTok{,}\DecValTok{1341927}\NormalTok{,}\DecValTok{1309799}\NormalTok{,}\DecValTok{1346686}\NormalTok{,}\DecValTok{1353776}\NormalTok{,}\DecValTok{1363749}\NormalTok{,}\DecValTok{956874}\NormalTok{,}\DecValTok{1344965}\NormalTok{,}\DecValTok{1373928}\NormalTok{,}\DecValTok{974166}\NormalTok{,}\DecValTok{978555}\NormalTok{,}\DecValTok{1347384}\NormalTok{,}\DecValTok{1326012}\NormalTok{,}\DecValTok{1386957}\NormalTok{,}\DecValTok{1308216}\NormalTok{,}\DecValTok{1367500}\NormalTok{,}\DecValTok{1305447}\NormalTok{,}\DecValTok{907013}\NormalTok{,}\DecValTok{1307046}\NormalTok{,}\DecValTok{1309068}\NormalTok{,}\DecValTok{1310756}\NormalTok{,}\DecValTok{1313200}\NormalTok{,}\DecValTok{1314577}\NormalTok{,}\DecValTok{1318137}\NormalTok{,}\DecValTok{1318853}\NormalTok{,}\DecValTok{1319880}\NormalTok{,}\DecValTok{40226742}\NormalTok{,}\DecValTok{1327978}\NormalTok{,}\DecValTok{1373225}\NormalTok{,}\DecValTok{1345858}\NormalTok{,}\DecValTok{1350489}\NormalTok{,}\DecValTok{1353766}\NormalTok{,}\DecValTok{1331235}\NormalTok{,}\DecValTok{1334456}\NormalTok{,}\DecValTok{970250}\NormalTok{,}\DecValTok{1317640}\NormalTok{,}\DecValTok{1341238}\NormalTok{,}\DecValTok{942350}\NormalTok{,}\DecValTok{1342439}\NormalTok{,}\DecValTok{904542}\NormalTok{,}\DecValTok{1308842}\NormalTok{,}\DecValTok{1307863}\NormalTok{)}
  \KeywordTok{and}\NormalTok{ c.invalid_reason }\KeywordTok{is} \KeywordTok{null}

\NormalTok{) I}
\NormalTok{) C;}
\KeywordTok{INSERT} \KeywordTok{INTO}\NormalTok{ #Codesets (codeset_id, concept_id)}
\KeywordTok{SELECT} \DecValTok{3} \KeywordTok{as}\NormalTok{ codeset_id, c.concept_id }\KeywordTok{FROM}\NormalTok{ (}\KeywordTok{select} \KeywordTok{distinct}\NormalTok{ I.concept_id }\KeywordTok{FROM}
\NormalTok{( }
  \KeywordTok{select}\NormalTok{ concept_id }\KeywordTok{from}\NormalTok{ @vocabulary_database_schema.CONCEPT }\KeywordTok{where}\NormalTok{ concept_id }\KeywordTok{in}\NormalTok{ (}\DecValTok{1335471}\NormalTok{,}\DecValTok{1340128}\NormalTok{,}\DecValTok{1341927}\NormalTok{,}\DecValTok{1363749}\NormalTok{,}\DecValTok{1308216}\NormalTok{,}\DecValTok{1310756}\NormalTok{,}\DecValTok{1373225}\NormalTok{,}\DecValTok{1331235}\NormalTok{,}\DecValTok{1334456}\NormalTok{,}\DecValTok{1342439}\NormalTok{)}
\KeywordTok{UNION}  \KeywordTok{select}\NormalTok{ c.concept_id}
  \KeywordTok{from}\NormalTok{ @vocabulary_database_schema.CONCEPT c}
  \KeywordTok{join}\NormalTok{ @vocabulary_database_schema.CONCEPT_ANCESTOR ca }\KeywordTok{on}\NormalTok{ c.concept_id }\OperatorTok{=}\NormalTok{ ca.descendant_concept_id}
  \KeywordTok{and}\NormalTok{ ca.ancestor_concept_id }\KeywordTok{in}\NormalTok{ (}\DecValTok{1335471}\NormalTok{,}\DecValTok{1340128}\NormalTok{,}\DecValTok{1341927}\NormalTok{,}\DecValTok{1363749}\NormalTok{,}\DecValTok{1308216}\NormalTok{,}\DecValTok{1310756}\NormalTok{,}\DecValTok{1373225}\NormalTok{,}\DecValTok{1331235}\NormalTok{,}\DecValTok{1334456}\NormalTok{,}\DecValTok{1342439}\NormalTok{)}
  \KeywordTok{and}\NormalTok{ c.invalid_reason }\KeywordTok{is} \KeywordTok{null}
...
\KeywordTok{with}\NormalTok{ primary_events (event_id, person_id, start_date, end_date, op_start_date, op_end_date, visit_occurrence_id) }\KeywordTok{as}
\NormalTok{(}
\CommentTok{-- Begin Primary Events}
\KeywordTok{select}\NormalTok{ P.ordinal }\KeywordTok{as}\NormalTok{ event_id, P.person_id, P.start_date, P.end_date, op_start_date, op_end_date, }\FunctionTok{cast}\NormalTok{(P.visit_occurrence_id }\KeywordTok{as}\NormalTok{ bigint) }\KeywordTok{as}\NormalTok{ visit_occurrence_id}
\KeywordTok{FROM}
\NormalTok{(}
  \KeywordTok{select}\NormalTok{ E.person_id, E.start_date, E.end_date, }\FunctionTok{row_number}\NormalTok{() }\KeywordTok{OVER}\NormalTok{ (}\KeywordTok{PARTITION} \KeywordTok{BY}\NormalTok{ E.person_id }\KeywordTok{ORDER} \KeywordTok{BY}\NormalTok{ E.start_date }\KeywordTok{ASC}\NormalTok{) ordinal, OP.observation_period_start_date }\KeywordTok{as}\NormalTok{ op_start_date, OP.observation_period_end_date }\KeywordTok{as}\NormalTok{ op_end_date, }\FunctionTok{cast}\NormalTok{(E.visit_occurrence_id }\KeywordTok{as}\NormalTok{ bigint) }\KeywordTok{as}\NormalTok{ visit_occurrence_id}
  \KeywordTok{FROM} 
\NormalTok{  (}
  \CommentTok{-- Begin Drug Exposure Criteria}
\KeywordTok{select}\NormalTok{ C.person_id, C.drug_exposure_id }\KeywordTok{as}\NormalTok{ event_id, C.drug_exposure_start_date }\KeywordTok{as}\NormalTok{ start_date, }\FunctionTok{COALESCE}\NormalTok{(C.drug_exposure_end_date, DATEADD(}\DataTypeTok{day}\NormalTok{, }\DecValTok{1}\NormalTok{, C.drug_exposure_start_date)) }\KeywordTok{as}\NormalTok{ end_date, C.drug_concept_id }\KeywordTok{as}\NormalTok{ TARGET_CONCEPT_ID, C.visit_occurrence_id}
\KeywordTok{from} 
\NormalTok{(}
  \KeywordTok{select}\NormalTok{ de.}\OperatorTok{*}\NormalTok{ , }\FunctionTok{row_number}\NormalTok{() }\KeywordTok{over}\NormalTok{ (}\KeywordTok{PARTITION} \KeywordTok{BY}\NormalTok{ de.person_id }\KeywordTok{ORDER} \KeywordTok{BY}\NormalTok{ de.drug_exposure_start_date, de.drug_exposure_id) }\KeywordTok{as}\NormalTok{ ordinal}
  \KeywordTok{FROM}\NormalTok{ @cdm_database_schema.DRUG_EXPOSURE de}
\KeywordTok{JOIN}\NormalTok{ #Codesets codesets }\KeywordTok{on}\NormalTok{ ((de.drug_concept_id }\OperatorTok{=}\NormalTok{ codesets.concept_id }\KeywordTok{and}\NormalTok{ codesets.codeset_id }\OperatorTok{=} \DecValTok{3}\NormalTok{))}
\NormalTok{) C}

\KeywordTok{WHERE}\NormalTok{ C.ordinal }\OperatorTok{=} \DecValTok{1}
\CommentTok{-- End Drug Exposure Criteria}

\NormalTok{  ) E}
    \KeywordTok{JOIN}\NormalTok{ @cdm_database_schema.observation_period OP }\KeywordTok{on}\NormalTok{ E.person_id }\OperatorTok{=}\NormalTok{ OP.person_id }\KeywordTok{and}\NormalTok{ E.start_date }\OperatorTok{>=}\NormalTok{  OP.observation_period_start_date }\KeywordTok{and}\NormalTok{ E.start_date }\OperatorTok{<=}\NormalTok{ op.observation_period_end_date}
  \KeywordTok{WHERE}\NormalTok{ DATEADD(}\DataTypeTok{day}\NormalTok{,}\DecValTok{365}\NormalTok{,OP.OBSERVATION_PERIOD_START_DATE) }\OperatorTok{<=}\NormalTok{ E.START_DATE }\KeywordTok{AND}\NormalTok{ DATEADD(}\DataTypeTok{day}\NormalTok{,}\DecValTok{0}\NormalTok{,E.START_DATE) }\OperatorTok{<=}\NormalTok{ OP.OBSERVATION_PERIOD_END_DATE}
\NormalTok{) P}
\KeywordTok{WHERE}\NormalTok{ P.ordinal }\OperatorTok{=} \DecValTok{1}
\CommentTok{-- End Primary Events}

\NormalTok{)}
\KeywordTok{SELECT}\NormalTok{ event_id, person_id, start_date, end_date, op_start_date, op_end_date, visit_occurrence_id}
\KeywordTok{INTO}\NormalTok{ #qualified_events}
\KeywordTok{FROM} 
\NormalTok{(}
  \KeywordTok{select}\NormalTok{ pe.event_id, pe.person_id, pe.start_date, pe.end_date, pe.op_start_date, pe.op_end_date, }\FunctionTok{row_number}\NormalTok{() }\KeywordTok{over}\NormalTok{ (}\KeywordTok{partition} \KeywordTok{by}\NormalTok{ pe.person_id }\KeywordTok{order} \KeywordTok{by}\NormalTok{ pe.start_date }\KeywordTok{ASC}\NormalTok{) }\KeywordTok{as}\NormalTok{ ordinal, }\FunctionTok{cast}\NormalTok{(pe.visit_occurrence_id }\KeywordTok{as}\NormalTok{ bigint) }\KeywordTok{as}\NormalTok{ visit_occurrence_id}
  \KeywordTok{FROM}\NormalTok{ primary_events pe}
  
\NormalTok{) QE}

\NormalTok{;}

\CommentTok{--- Inclusion Rule Inserts}

\KeywordTok{select} \DecValTok{0} \KeywordTok{as}\NormalTok{ inclusion_rule_id, person_id, event_id}
\KeywordTok{INTO}\NormalTok{ #Inclusion_0}
\KeywordTok{FROM} 
\NormalTok{(}
  \KeywordTok{select}\NormalTok{ pe.person_id, pe.event_id}
  \KeywordTok{FROM}\NormalTok{ #qualified_events pe}
  
\KeywordTok{JOIN}\NormalTok{ (}
\CommentTok{-- Begin Criteria Group}
\KeywordTok{select} \DecValTok{0} \KeywordTok{as}\NormalTok{ index_id, person_id, event_id}
\KeywordTok{FROM}
\NormalTok{(}
  \KeywordTok{select}\NormalTok{ E.person_id, E.event_id }
  \KeywordTok{FROM}\NormalTok{ #qualified_events E}
  \KeywordTok{INNER} \KeywordTok{JOIN}
\NormalTok{  (}
    \CommentTok{-- Begin Correlated Criteria}
\KeywordTok{SELECT} \DecValTok{0} \KeywordTok{as}\NormalTok{ index_id, p.person_id, p.event_id}
\KeywordTok{FROM}\NormalTok{ #qualified_events P}
\KeywordTok{INNER} \KeywordTok{JOIN}
\NormalTok{(}
  \CommentTok{-- Begin Condition Occurrence Criteria}
\KeywordTok{SELECT}\NormalTok{ C.person_id, C.condition_occurrence_id }\KeywordTok{as}\NormalTok{ event_id, C.condition_start_date }\KeywordTok{as}\NormalTok{ start_date, }\FunctionTok{COALESCE}\NormalTok{(C.condition_end_date, DATEADD(}\DataTypeTok{day}\NormalTok{,}\DecValTok{1}\NormalTok{,C.condition_start_date)) }\KeywordTok{as}\NormalTok{ end_date, C.CONDITION_CONCEPT_ID }\KeywordTok{as}\NormalTok{ TARGET_CONCEPT_ID, C.visit_occurrence_id}
\KeywordTok{FROM} 
\NormalTok{(}
  \KeywordTok{SELECT}\NormalTok{ co.}\OperatorTok{*} 
  \KeywordTok{FROM}\NormalTok{ @cdm_database_schema.CONDITION_OCCURRENCE co}
  \KeywordTok{JOIN}\NormalTok{ #Codesets codesets }\KeywordTok{on}\NormalTok{ ((co.condition_concept_id }\OperatorTok{=}\NormalTok{ codesets.concept_id }\KeywordTok{and}\NormalTok{ codesets.codeset_id }\OperatorTok{=} \DecValTok{0}\NormalTok{))}
\NormalTok{) C}


\CommentTok{-- End Condition Occurrence Criteria}

\NormalTok{) A }\KeywordTok{on}\NormalTok{ A.person_id }\OperatorTok{=}\NormalTok{ P.person_id  }\KeywordTok{AND}\NormalTok{ A.START_DATE }\OperatorTok{>=}\NormalTok{ P.OP_START_DATE }\KeywordTok{AND}\NormalTok{ A.START_DATE }\OperatorTok{<=}\NormalTok{ P.OP_END_DATE }\KeywordTok{AND}\NormalTok{ A.START_DATE }\OperatorTok{>=}\NormalTok{ DATEADD(}\DataTypeTok{day}\NormalTok{,}\OperatorTok{-}\DecValTok{365}\NormalTok{,P.START_DATE) }\KeywordTok{AND}\NormalTok{ A.START_DATE }\OperatorTok{<=}\NormalTok{ DATEADD(}\DataTypeTok{day}\NormalTok{,}\DecValTok{0}\NormalTok{,P.START_DATE)}
\KeywordTok{GROUP} \KeywordTok{BY}\NormalTok{ p.person_id, p.event_id}
\KeywordTok{HAVING} \FunctionTok{COUNT}\NormalTok{(A.TARGET_CONCEPT_ID) }\OperatorTok{>=} \DecValTok{1}
\CommentTok{-- End Correlated Criteria}

\NormalTok{  ) CQ }\KeywordTok{on}\NormalTok{ E.person_id }\OperatorTok{=}\NormalTok{ CQ.person_id }\KeywordTok{and}\NormalTok{ E.event_id }\OperatorTok{=}\NormalTok{ CQ.event_id}
  \KeywordTok{GROUP} \KeywordTok{BY}\NormalTok{ E.person_id, E.event_id}
  \KeywordTok{HAVING} \FunctionTok{COUNT}\NormalTok{(index_id) }\OperatorTok{=} \DecValTok{1}
\NormalTok{) G}
\CommentTok{-- End Criteria Group}
\NormalTok{) AC }\KeywordTok{on}\NormalTok{ AC.person_id }\OperatorTok{=}\NormalTok{ pe.person_id }\KeywordTok{AND}\NormalTok{ AC.event_id }\OperatorTok{=}\NormalTok{ pe.event_id}
\NormalTok{) Results}
\NormalTok{;}

\KeywordTok{select} \DecValTok{1} \KeywordTok{as}\NormalTok{ inclusion_rule_id, person_id, event_id}
\KeywordTok{INTO}\NormalTok{ #Inclusion_1}
\KeywordTok{FROM} 
\NormalTok{(}
  \KeywordTok{select}\NormalTok{ pe.person_id, pe.event_id}
  \KeywordTok{FROM}\NormalTok{ #qualified_events pe}
...
\KeywordTok{from} 
\NormalTok{(}
  \KeywordTok{select}\NormalTok{ de.}\OperatorTok{*} 
  \KeywordTok{FROM}\NormalTok{ @cdm_database_schema.DRUG_EXPOSURE de}
\KeywordTok{JOIN}\NormalTok{ #Codesets codesets }\KeywordTok{on}\NormalTok{ ((de.drug_concept_id }\OperatorTok{=}\NormalTok{ codesets.concept_id }\KeywordTok{and}\NormalTok{ codesets.codeset_id }\OperatorTok{=} \DecValTok{2}\NormalTok{))}
\NormalTok{) C}


\CommentTok{-- End Drug Exposure Criteria}

\NormalTok{) A }\KeywordTok{on}\NormalTok{ A.person_id }\OperatorTok{=}\NormalTok{ P.person_id  }\KeywordTok{AND}\NormalTok{ A.START_DATE }\OperatorTok{>=}\NormalTok{ P.OP_START_DATE }\KeywordTok{AND}\NormalTok{ A.START_DATE }\OperatorTok{<=}\NormalTok{ P.OP_END_DATE }\KeywordTok{AND}\NormalTok{ A.START_DATE }\OperatorTok{>=}\NormalTok{ P.OP_START_DATE }\KeywordTok{AND}\NormalTok{ A.START_DATE }\OperatorTok{<=}\NormalTok{ DATEADD(}\DataTypeTok{day}\NormalTok{,}\OperatorTok{-}\DecValTok{1}\NormalTok{,P.START_DATE)}
\KeywordTok{GROUP} \KeywordTok{BY}\NormalTok{ p.person_id, p.event_id}
\KeywordTok{HAVING} \FunctionTok{COUNT}\NormalTok{(A.TARGET_CONCEPT_ID) }\OperatorTok{=} \DecValTok{0}
\CommentTok{-- End Correlated Criteria}
...
\KeywordTok{select} \DecValTok{2} \KeywordTok{as}\NormalTok{ inclusion_rule_id, person_id, event_id}
\KeywordTok{INTO}\NormalTok{ #Inclusion_2}
\KeywordTok{FROM} 
\NormalTok{(}
  \KeywordTok{select}\NormalTok{ pe.person_id, pe.event_id}
  \KeywordTok{FROM}\NormalTok{ #qualified_events pe}
...
\KeywordTok{INNER} \KeywordTok{JOIN}
\NormalTok{(}
  \CommentTok{-- Begin Drug Era Criteria}
\KeywordTok{select}\NormalTok{ C.person_id, C.drug_era_id }\KeywordTok{as}\NormalTok{ event_id, C.drug_era_start_date }\KeywordTok{as}\NormalTok{ start_date, C.drug_era_end_date }\KeywordTok{as}\NormalTok{ end_date, C.drug_concept_id }\KeywordTok{as}\NormalTok{ TARGET_CONCEPT_ID, }\FunctionTok{CAST}\NormalTok{(}\KeywordTok{NULL} \KeywordTok{as}\NormalTok{ bigint) }\KeywordTok{as}\NormalTok{ visit_occurrence_id}
\KeywordTok{from} 
\NormalTok{(}
  \KeywordTok{select}\NormalTok{ de.}\OperatorTok{*} 
  \KeywordTok{FROM}\NormalTok{ @cdm_database_schema.DRUG_ERA de}
\KeywordTok{where}\NormalTok{ de.drug_concept_id }\KeywordTok{in}\NormalTok{ (}\KeywordTok{SELECT}\NormalTok{ concept_id }\KeywordTok{from}\NormalTok{  #Codesets }\KeywordTok{where}\NormalTok{ codeset_id }\OperatorTok{=} \DecValTok{2}\NormalTok{)}
\NormalTok{) C}


\CommentTok{-- End Drug Era Criteria}

\NormalTok{) A }\KeywordTok{on}\NormalTok{ A.person_id }\OperatorTok{=}\NormalTok{ P.person_id  }\KeywordTok{AND}\NormalTok{ A.START_DATE }\OperatorTok{>=}\NormalTok{ P.OP_START_DATE }\KeywordTok{AND}\NormalTok{ A.START_DATE }\OperatorTok{<=}\NormalTok{ P.OP_END_DATE }\KeywordTok{AND}\NormalTok{ A.START_DATE }\OperatorTok{>=}\NormalTok{ DATEADD(}\DataTypeTok{day}\NormalTok{,}\DecValTok{0}\NormalTok{,P.START_DATE) }\KeywordTok{AND}\NormalTok{ A.START_DATE }\OperatorTok{<=}\NormalTok{ DATEADD(}\DataTypeTok{day}\NormalTok{,}\DecValTok{7}\NormalTok{,P.START_DATE)}
\KeywordTok{GROUP} \KeywordTok{BY}\NormalTok{ p.person_id, p.event_id}
\KeywordTok{HAVING} \FunctionTok{COUNT}\NormalTok{(}\KeywordTok{DISTINCT}\NormalTok{ A.TARGET_CONCEPT_ID) }\OperatorTok{=} \DecValTok{1}
\CommentTok{-- End Correlated Criteria}
...
\KeywordTok{SELECT}\NormalTok{ inclusion_rule_id, person_id, event_id}
\KeywordTok{INTO}\NormalTok{ #inclusion_events}
\KeywordTok{FROM}\NormalTok{ (}\KeywordTok{select}\NormalTok{ inclusion_rule_id, person_id, event_id }\KeywordTok{from}\NormalTok{ #Inclusion_0}
\KeywordTok{UNION} \KeywordTok{ALL}
\KeywordTok{select}\NormalTok{ inclusion_rule_id, person_id, event_id }\KeywordTok{from}\NormalTok{ #Inclusion_1}
\KeywordTok{UNION} \KeywordTok{ALL}
\KeywordTok{select}\NormalTok{ inclusion_rule_id, person_id, event_id }\KeywordTok{from}\NormalTok{ #Inclusion_2) I;}
\KeywordTok{TRUNCATE} \KeywordTok{TABLE}\NormalTok{ #Inclusion_0;}
\KeywordTok{DROP} \KeywordTok{TABLE}\NormalTok{ #Inclusion_0;}

\KeywordTok{TRUNCATE} \KeywordTok{TABLE}\NormalTok{ #Inclusion_1;}
\KeywordTok{DROP} \KeywordTok{TABLE}\NormalTok{ #Inclusion_1;}

\KeywordTok{TRUNCATE} \KeywordTok{TABLE}\NormalTok{ #Inclusion_2;}
\KeywordTok{DROP} \KeywordTok{TABLE}\NormalTok{ #Inclusion_2;}


\KeywordTok{with}\NormalTok{ cteIncludedEvents(event_id, person_id, start_date, end_date, op_start_date, op_end_date, ordinal) }\KeywordTok{as}
\NormalTok{(}
  \KeywordTok{SELECT}\NormalTok{ event_id, person_id, start_date, end_date, op_start_date, op_end_date, }\FunctionTok{row_number}\NormalTok{() }\KeywordTok{over}\NormalTok{ (}\KeywordTok{partition} \KeywordTok{by}\NormalTok{ person_id }\KeywordTok{order} \KeywordTok{by}\NormalTok{ start_date }\KeywordTok{ASC}\NormalTok{) }\KeywordTok{as}\NormalTok{ ordinal}
  \KeywordTok{from}
\NormalTok{  (}
    \KeywordTok{select}\NormalTok{ Q.event_id, Q.person_id, Q.start_date, Q.end_date, Q.op_start_date, Q.op_end_date, }\FunctionTok{SUM}\NormalTok{(}\FunctionTok{coalesce}\NormalTok{(}\FunctionTok{POWER}\NormalTok{(}\FunctionTok{cast}\NormalTok{(}\DecValTok{2} \KeywordTok{as}\NormalTok{ bigint), I.inclusion_rule_id), }\DecValTok{0}\NormalTok{)) }\KeywordTok{as}\NormalTok{ inclusion_rule_mask}
    \KeywordTok{from}\NormalTok{ #qualified_events Q}
    \KeywordTok{LEFT} \KeywordTok{JOIN}\NormalTok{ #inclusion_events I }\KeywordTok{on}\NormalTok{ I.person_id }\OperatorTok{=}\NormalTok{ Q.person_id }\KeywordTok{and}\NormalTok{ I.event_id }\OperatorTok{=}\NormalTok{ Q.event_id}
    \KeywordTok{GROUP} \KeywordTok{BY}\NormalTok{ Q.event_id, Q.person_id, Q.start_date, Q.end_date, Q.op_start_date, Q.op_end_date}
\NormalTok{  ) MG }\CommentTok{-- matching groups}

  \CommentTok{-- the matching group with all bits set ( POWER(2,# of inclusion rules) - 1 = inclusion_rule_mask}
  \KeywordTok{WHERE}\NormalTok{ (MG.inclusion_rule_mask }\OperatorTok{=} \FunctionTok{POWER}\NormalTok{(}\FunctionTok{cast}\NormalTok{(}\DecValTok{2} \KeywordTok{as}\NormalTok{ bigint),}\DecValTok{3}\NormalTok{)}\OperatorTok{-}\DecValTok{1}\NormalTok{)}

\NormalTok{)}
\KeywordTok{select}\NormalTok{ event_id, person_id, start_date, end_date, op_start_date, op_end_date}
\KeywordTok{into}\NormalTok{ #included_events}
\KeywordTok{FROM}\NormalTok{ cteIncludedEvents Results}
\KeywordTok{WHERE}\NormalTok{ Results.ordinal }\OperatorTok{=} \DecValTok{1}
\NormalTok{;}

\CommentTok{-- custom era strategy}

\KeywordTok{with}\NormalTok{ ctePersons(person_id) }\KeywordTok{as}\NormalTok{ (}
    \KeywordTok{select} \KeywordTok{distinct}\NormalTok{ person_id }\KeywordTok{from}\NormalTok{ #included_events}
\NormalTok{)}

\KeywordTok{select}\NormalTok{ person_id, drug_exposure_start_date, drug_exposure_end_date}
\KeywordTok{INTO}\NormalTok{ #drugTarget}
\KeywordTok{FROM}\NormalTok{ (}
    \KeywordTok{select}\NormalTok{ de.PERSON_ID, DRUG_EXPOSURE_START_DATE,  }\FunctionTok{COALESCE}\NormalTok{(DRUG_EXPOSURE_END_DATE, DATEADD(}\DataTypeTok{day}\NormalTok{,DAYS_SUPPLY,DRUG_EXPOSURE_START_DATE), DATEADD(}\DataTypeTok{day}\NormalTok{,}\DecValTok{1}\NormalTok{,DRUG_EXPOSURE_START_DATE)) }\KeywordTok{as}\NormalTok{ DRUG_EXPOSURE_END_DATE }
    \KeywordTok{FROM}\NormalTok{ @cdm_database_schema.DRUG_EXPOSURE de}
    \KeywordTok{JOIN}\NormalTok{ ctePersons p }\KeywordTok{on}\NormalTok{ de.person_id }\OperatorTok{=}\NormalTok{ p.person_id}
    \KeywordTok{JOIN}\NormalTok{ #Codesets cs }\KeywordTok{on}\NormalTok{ cs.codeset_id }\OperatorTok{=} \DecValTok{3} \KeywordTok{AND}\NormalTok{ de.drug_concept_id }\OperatorTok{=}\NormalTok{ cs.concept_id}
    ...
    \KeywordTok{FROM}\NormalTok{ @cdm_database_schema.DRUG_EXPOSURE de}
    \KeywordTok{JOIN}\NormalTok{ ctePersons p }\KeywordTok{on}\NormalTok{ de.person_id }\OperatorTok{=}\NormalTok{ p.person_id}
    \KeywordTok{JOIN}\NormalTok{ #Codesets cs }\KeywordTok{on}\NormalTok{ cs.codeset_id }\OperatorTok{=} \DecValTok{3} \KeywordTok{AND}\NormalTok{ de.drug_source_concept_id }\OperatorTok{=}\NormalTok{ cs.concept_id}
\NormalTok{) E}
\NormalTok{;}

\KeywordTok{select}\NormalTok{ et.event_id, et.person_id, ERAS.era_end_date }\KeywordTok{as}\NormalTok{ end_date}
\KeywordTok{INTO}\NormalTok{ #strategy_ends}
\KeywordTok{from}\NormalTok{ #included_events et}
\KeywordTok{JOIN} 
\NormalTok{(}
  \KeywordTok{select}\NormalTok{ ENDS.person_id, }\FunctionTok{min}\NormalTok{(drug_exposure_start_date) }\KeywordTok{as}\NormalTok{ era_start_date, DATEADD(}\DataTypeTok{day}\NormalTok{,}\DecValTok{0}\NormalTok{, ENDS.era_end_date) }\KeywordTok{as}\NormalTok{ era_end_date}
  \KeywordTok{from}
\NormalTok{  (}
    \KeywordTok{select}\NormalTok{ de.person_id, de.drug_exposure_start_date, }\FunctionTok{MIN}\NormalTok{(e.END_DATE) }\KeywordTok{as}\NormalTok{ era_end_date}
    \KeywordTok{FROM}\NormalTok{ #drugTarget DE}
    \KeywordTok{JOIN} 
\NormalTok{    (}
      \CommentTok{--cteEndDates}
      \KeywordTok{select}\NormalTok{ PERSON_ID, DATEADD(}\DataTypeTok{day}\NormalTok{,}\OperatorTok{-}\DecValTok{1} \OperatorTok{*} \DecValTok{30}\NormalTok{,EVENT_DATE) }\KeywordTok{as}\NormalTok{ END_DATE }\CommentTok{-- unpad the end date by 30}
      \KeywordTok{FROM}
\NormalTok{      (}
                \KeywordTok{select}\NormalTok{ PERSON_ID, EVENT_DATE, EVENT_TYPE, }
                \FunctionTok{MAX}\NormalTok{(START_ORDINAL) }\KeywordTok{OVER}\NormalTok{ (}\KeywordTok{PARTITION} \KeywordTok{BY}\NormalTok{ PERSON_ID }\KeywordTok{ORDER} \KeywordTok{BY}\NormalTok{ event_date, event_type }\KeywordTok{ROWS} \KeywordTok{UNBOUNDED} \KeywordTok{PRECEDING}\NormalTok{) }\KeywordTok{AS}\NormalTok{ start_ordinal,}
                \FunctionTok{ROW_NUMBER}\NormalTok{() }\KeywordTok{OVER}\NormalTok{ (}\KeywordTok{PARTITION} \KeywordTok{BY}\NormalTok{ PERSON_ID }\KeywordTok{ORDER} \KeywordTok{BY}\NormalTok{ EVENT_DATE, EVENT_TYPE) }\KeywordTok{AS}\NormalTok{ OVERALL_ORD }\CommentTok{-- this re-numbers the inner UNION so all rows are numbered ordered by the event date}
                \KeywordTok{from}
\NormalTok{                (}
                    \CommentTok{-- select the start dates, assigning a row number to each}
                    \KeywordTok{Select}\NormalTok{ PERSON_ID, DRUG_EXPOSURE_START_DATE }\KeywordTok{AS}\NormalTok{ EVENT_DATE, }\DecValTok{0} \KeywordTok{as}\NormalTok{ EVENT_TYPE, }\FunctionTok{ROW_NUMBER}\NormalTok{() }\KeywordTok{OVER}\NormalTok{ (}\KeywordTok{PARTITION} \KeywordTok{BY}\NormalTok{ PERSON_ID }\KeywordTok{ORDER} \KeywordTok{BY}\NormalTok{ DRUG_EXPOSURE_START_DATE) }\KeywordTok{as}\NormalTok{ START_ORDINAL}
                    \KeywordTok{from}\NormalTok{ #drugTarget D}

                    \KeywordTok{UNION} \KeywordTok{ALL}

                    \CommentTok{-- add the end dates with NULL as the row number, padding the end dates by 30 to allow a grace period for overlapping ranges.}
                    \KeywordTok{select}\NormalTok{ PERSON_ID, DATEADD(}\DataTypeTok{day}\NormalTok{,}\DecValTok{30}\NormalTok{,DRUG_EXPOSURE_END_DATE), }\DecValTok{1} \KeywordTok{as}\NormalTok{ EVENT_TYPE, }\KeywordTok{NULL}
                    \KeywordTok{FROM}\NormalTok{ #drugTarget D}
\NormalTok{                ) RAWDATA}
\NormalTok{      ) E}
      \KeywordTok{WHERE} \DecValTok{2} \OperatorTok{*}\NormalTok{ E.START_ORDINAL }\OperatorTok{-}\NormalTok{ E.OVERALL_ORD }\OperatorTok{=} \DecValTok{0}
\NormalTok{    ) E }\KeywordTok{on}\NormalTok{ DE.PERSON_ID }\OperatorTok{=}\NormalTok{ E.PERSON_ID }\KeywordTok{and}\NormalTok{ E.END_DATE }\OperatorTok{>=}\NormalTok{ DE.DRUG_EXPOSURE_START_DATE}
    \KeywordTok{GROUP} \KeywordTok{BY}\NormalTok{ de.person_id, de.drug_exposure_start_date}
\NormalTok{  ) ENDS}
  \KeywordTok{GROUP} \KeywordTok{BY}\NormalTok{ ENDS.person_id, ENDS.era_end_date}
\NormalTok{) ERAS }\KeywordTok{on}\NormalTok{ ERAS.person_id }\OperatorTok{=}\NormalTok{ et.person_id }
\KeywordTok{WHERE}\NormalTok{ et.start_date }\KeywordTok{between}\NormalTok{ ERAS.era_start_date }\KeywordTok{and}\NormalTok{ ERAS.era_end_date;}

\KeywordTok{TRUNCATE} \KeywordTok{TABLE}\NormalTok{ #drugTarget;}
\KeywordTok{DROP} \KeywordTok{TABLE}\NormalTok{ #drugTarget;}


\CommentTok{-- generate cohort periods into #final_cohort}
\KeywordTok{with}\NormalTok{ cohort_ends (event_id, person_id, end_date) }\KeywordTok{as}
\NormalTok{(}
    \CommentTok{-- cohort exit dates}
  \CommentTok{-- By default, cohort exit at the event's op end date}
\KeywordTok{select}\NormalTok{ event_id, person_id, op_end_date }\KeywordTok{as}\NormalTok{ end_date }\KeywordTok{from}\NormalTok{ #included_events}
\KeywordTok{UNION} \KeywordTok{ALL}
\CommentTok{-- End Date Strategy}
\KeywordTok{SELECT}\NormalTok{ event_id, person_id, end_date }\KeywordTok{from}\NormalTok{ #strategy_ends}

\NormalTok{),}
\NormalTok{first_ends (person_id, start_date, end_date) }\KeywordTok{as}
\NormalTok{(}
    \KeywordTok{select}\NormalTok{ F.person_id, F.start_date, F.end_date}
    \KeywordTok{FROM}\NormalTok{ (}
      \KeywordTok{select}\NormalTok{ I.event_id, I.person_id, I.start_date, E.end_date, }\FunctionTok{row_number}\NormalTok{() }\KeywordTok{over}\NormalTok{ (}\KeywordTok{partition} \KeywordTok{by}\NormalTok{ I.person_id, I.event_id }\KeywordTok{order} \KeywordTok{by}\NormalTok{ E.end_date) }\KeywordTok{as}\NormalTok{ ordinal }
      \KeywordTok{from}\NormalTok{ #included_events I}
      \KeywordTok{join}\NormalTok{ cohort_ends E }\KeywordTok{on}\NormalTok{ I.event_id }\OperatorTok{=}\NormalTok{ E.event_id }\KeywordTok{and}\NormalTok{ I.person_id }\OperatorTok{=}\NormalTok{ E.person_id }\KeywordTok{and}\NormalTok{ E.end_date }\OperatorTok{>=}\NormalTok{ I.start_date}
\NormalTok{    ) F}
    \KeywordTok{WHERE}\NormalTok{ F.ordinal }\OperatorTok{=} \DecValTok{1}
\NormalTok{)}
\KeywordTok{select}\NormalTok{ person_id, start_date, end_date}
\KeywordTok{INTO}\NormalTok{ #cohort_rows}
\KeywordTok{from}\NormalTok{ first_ends;}

\KeywordTok{with}\NormalTok{ cteEndDates (person_id, end_date) }\KeywordTok{AS} \CommentTok{-- the magic}
\NormalTok{(   }
    \KeywordTok{SELECT}
\NormalTok{        person_id}
\NormalTok{        , DATEADD(}\DataTypeTok{day}\NormalTok{,}\OperatorTok{-}\DecValTok{1} \OperatorTok{*} \DecValTok{0}\NormalTok{, event_date)  }\KeywordTok{as}\NormalTok{ end_date}
    \KeywordTok{FROM}
\NormalTok{    (}
        \KeywordTok{SELECT}
\NormalTok{            person_id}
\NormalTok{            , event_date}
\NormalTok{            , event_type}
\NormalTok{            , }\FunctionTok{MAX}\NormalTok{(start_ordinal) }\KeywordTok{OVER}\NormalTok{ (}\KeywordTok{PARTITION} \KeywordTok{BY}\NormalTok{ person_id }\KeywordTok{ORDER} \KeywordTok{BY}\NormalTok{ event_date, event_type }\KeywordTok{ROWS} \KeywordTok{UNBOUNDED} \KeywordTok{PRECEDING}\NormalTok{) }\KeywordTok{AS}\NormalTok{ start_ordinal }
\NormalTok{            , }\FunctionTok{ROW_NUMBER}\NormalTok{() }\KeywordTok{OVER}\NormalTok{ (}\KeywordTok{PARTITION} \KeywordTok{BY}\NormalTok{ person_id }\KeywordTok{ORDER} \KeywordTok{BY}\NormalTok{ event_date, event_type) }\KeywordTok{AS}\NormalTok{ overall_ord}
        \KeywordTok{FROM}
\NormalTok{        (}
            \KeywordTok{SELECT}
\NormalTok{                person_id}
\NormalTok{                , start_date }\KeywordTok{AS}\NormalTok{ event_date}
\NormalTok{                , }\OperatorTok{-}\DecValTok{1} \KeywordTok{AS}\NormalTok{ event_type}
\NormalTok{                , }\FunctionTok{ROW_NUMBER}\NormalTok{() }\KeywordTok{OVER}\NormalTok{ (}\KeywordTok{PARTITION} \KeywordTok{BY}\NormalTok{ person_id }\KeywordTok{ORDER} \KeywordTok{BY}\NormalTok{ start_date) }\KeywordTok{AS}\NormalTok{ start_ordinal}
            \KeywordTok{FROM}\NormalTok{ #cohort_rows}
            ...
            \KeywordTok{SELECT}
\NormalTok{                person_id}
\NormalTok{                , DATEADD(}\DataTypeTok{day}\NormalTok{,}\DecValTok{0}\NormalTok{,end_date) }\KeywordTok{as}\NormalTok{ end_date}
\NormalTok{                , }\DecValTok{1} \KeywordTok{AS}\NormalTok{ event_type}
\NormalTok{                , }\KeywordTok{NULL}
            \KeywordTok{FROM}\NormalTok{ #cohort_rows}
\NormalTok{        ) RAWDATA}
\NormalTok{    ) e}
    \KeywordTok{WHERE}\NormalTok{ (}\DecValTok{2} \OperatorTok{*}\NormalTok{ e.start_ordinal) }\OperatorTok{-}\NormalTok{ e.overall_ord }\OperatorTok{=} \DecValTok{0}
\NormalTok{),}
\NormalTok{cteEnds (person_id, start_date, end_date) }\KeywordTok{AS}
\NormalTok{(}
    \KeywordTok{SELECT}
\NormalTok{         c.person_id}
\NormalTok{        , c.start_date}
\NormalTok{        , }\FunctionTok{MIN}\NormalTok{(e.end_date) }\KeywordTok{AS}\NormalTok{ era_end_date}
    \KeywordTok{FROM}\NormalTok{ #cohort_rows c}
    \KeywordTok{JOIN}\NormalTok{ cteEndDates e }\KeywordTok{ON}\NormalTok{ c.person_id }\OperatorTok{=}\NormalTok{ e.person_id }\KeywordTok{AND}\NormalTok{ e.end_date }\OperatorTok{>=}\NormalTok{ c.start_date}
    \KeywordTok{GROUP} \KeywordTok{BY}\NormalTok{ c.person_id, c.start_date}
\NormalTok{)}
\KeywordTok{select}\NormalTok{ person_id, }\FunctionTok{min}\NormalTok{(start_date) }\KeywordTok{as}\NormalTok{ start_date, end_date}
\KeywordTok{into}\NormalTok{ #final_cohort}
\KeywordTok{from}\NormalTok{ cteEnds}
\KeywordTok{group} \KeywordTok{by}\NormalTok{ person_id, end_date}
\NormalTok{;}

\KeywordTok{DELETE} \KeywordTok{FROM}\NormalTok{ @target_database_schema.@target_cohort_table }\KeywordTok{where}\NormalTok{ cohort_definition_id }\OperatorTok{=}\NormalTok{ @target_cohort_id;}
\KeywordTok{INSERT} \KeywordTok{INTO}\NormalTok{ @target_database_schema.@target_cohort_table (cohort_definition_id, subject_id, cohort_start_date, cohort_end_date)}
\KeywordTok{select}\NormalTok{ @target_cohort_id }\KeywordTok{as}\NormalTok{ cohort_definition_id, person_id, start_date, end_date }
\KeywordTok{FROM}\NormalTok{ #final_cohort CO}
\NormalTok{;}



\KeywordTok{TRUNCATE} \KeywordTok{TABLE}\NormalTok{ #strategy_ends;}
\KeywordTok{DROP} \KeywordTok{TABLE}\NormalTok{ #strategy_ends;}


\KeywordTok{TRUNCATE} \KeywordTok{TABLE}\NormalTok{ #cohort_rows;}
\KeywordTok{DROP} \KeywordTok{TABLE}\NormalTok{ #cohort_rows;}

\KeywordTok{TRUNCATE} \KeywordTok{TABLE}\NormalTok{ #final_cohort;}
\KeywordTok{DROP} \KeywordTok{TABLE}\NormalTok{ #final_cohort;}

\KeywordTok{TRUNCATE} \KeywordTok{TABLE}\NormalTok{ #inclusion_events;}
\KeywordTok{DROP} \KeywordTok{TABLE}\NormalTok{ #inclusion_events;}

\KeywordTok{TRUNCATE} \KeywordTok{TABLE}\NormalTok{ #qualified_events;}
\KeywordTok{DROP} \KeywordTok{TABLE}\NormalTok{ #qualified_events;}

\KeywordTok{TRUNCATE} \KeywordTok{TABLE}\NormalTok{ #included_events;}
\KeywordTok{DROP} \KeywordTok{TABLE}\NormalTok{ #included_events;}

\KeywordTok{TRUNCATE} \KeywordTok{TABLE}\NormalTok{ #Codesets;}
\KeywordTok{DROP} \KeywordTok{TABLE}\NormalTok{ #Codesets;}
\end{Highlighting}
\end{Shaded}
...
\section{Patients initiating first-line therapy for hypertension with \textgreater1 yr follow-up}\label{HTN1yrFO}}
