...
\begin{verbatim}
## [1] "SELECT * FROM temp_schema.wsyljxulchildren ;"
\end{verbatim}
...
conditioning is to assume a separate estimate is produced in each
...
In Section @\{ComparisonSettings\} we selected a concept set
...
ATLAS provides standard logic, with three options. The first option is
whether to \textbf{use all occurrences} or just the \textbf{first
occurrence} of the concept. The second option determines \textbf{whether
...
option specifies which domains should be considered when looking for the
concepts.
...
cohort definitions which can be used to instantiate the target,
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{matchedPop <-}\StringTok{ }\KeywordTok{matchOnPs}\NormalTok{(}\DataTypeTok{population =}\NormalTok{ ps, }
