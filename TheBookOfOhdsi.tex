...
\begin{verbatim}
## [1] "SELECT * FROM temp_schema.z0vvvdkwchildren ;"
\end{verbatim}
...
Test each covariate for correlation with the target assignment? If any
covariate has an unusually high correlation (either positive or
negative), this will throw an error.

This avoids lengthy calculation of a propensity model only to discover
complete separation. Finding very high univariate correlation allows you
to review the covariate to determine why it has high correlation and
whether it should be dropped.

If an error occurs, should the function stop? Else, the two cohorts will
be assumed to be perfectly separable.

Yes, stop, as there is normally no sense in carrying out the
calculations and not have a usable result due to perfect separation.
