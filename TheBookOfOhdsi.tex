...
CDM. We close this chapter with an example study estimating incidence
rates, and implement this study using SqlRender and DatabaseConnector.
...
\begin{verbatim}
## [1] "SELECT * FROM temp_schema.wu0k5xz3children ;"
\end{verbatim}
...
descendants, ie. all drugs containing an ACEi. We take the first drug
...
use snake\_case for field names (because SQL in case insentive), whereas
in R we tend to use camelCase (because R is case sensitive). The
\texttt{results} data frame colum names will now be in camelCase.
...
Note that for demonstration purposes we chose to create our cohortst
