...
In this chapter we describe OHDSI’s standardized framework for patient-level prediction, [@reps2018] and discuss in detail the [PatientLevelPrediction](https://ohdsi.github.io/PatientLevelPrediction/) R package that implements all established best practices. We start with providing the necessary theory behind the development and evaluation of patient-level prediction and provide a high-level overview of the implemented machine learning algorithms. We then discuss an example prediction problem and provide step-by-step guidance on its definition and implementation using the powerful ATLAS tool and custom R code. Finally, we review two Shiny apps that allow view the study outputs. One app explores a single prediction model, while the other explores many models.
...
> Among *[target cohort definition]*, who will go on to have *[outcome definition]* within *[time-at-risk period]*?

Furthermore, we have to make design choices for the model we like to develop, and determine the observational data sets to perform internal and external validation. 
...
- Treatment choice
  - **Structure**: Among patients with *[indicated disease]* who are treated with either *[treatment 1]* or *[treatment 2]*, which patients were treated with *[treatment 1]* (on day 0).
...
- Treatment safety
  ...
  - **Example**: Among new users of warfarin, who will have a GI bleed in one year?
- Treatment adherence
...
2. A set describing the **outcome status** (also referred to as the labels or classes to predict). Did a patient experience the outcome of interest in the time-at-risk?
...
We can evaluate a prediction model by measuring the agreement between the model's prediction and observed outcome status, which means we need data where the outcome status is known. We must use a different data set than was used to develop the model, or else we run the risk of favoring models that are over-fitted (see Section \@ref(modelFitting)) and may not perform well for new patients. We distinguish between 
...
- A **holdout set** approach simply splits the labelled data into two independent sets, a train set and a test set (the hold out set). The train set is used to learn the model and the test set is used to evaluate it. We can simply divide our patients randomly into a train and test set, or we may choose to:
...
- **Cross validation** is useful when the data are limited. The data is split into $n$ equally-sized sets, where $n$ needs to be prespecified, for example $n=10$. For each of these sets a model is trained on all data except the data in set, and used to generate predictions for the held-out set. In this way, all data is used once to evaluate the model-building algorithm. In the patient-level prediction framework we use cross validation to pick the optimal hyper-parameters on the train set.
...
To develop the model we have to decide which algorithm(s) we like to train. We see the selection of the best algorithm for a certain prediction problem as an empirical question, i.e. we prefer to let the data speak for itself and try different approaches to find the best one. There is no algorithm that will work best for all problems (no free lunch). In our framework we have therefore implement many algorithms as described in Section \@ref(modelFitting), and allow others to be added. This out-of-scope for this chapter but mode details can be found in the ["Adding Custom Patient-Level Prediction Algorithms" vignette](https://ohdsi.github.io/PatientLevelPrediction/articles/AddingCustomAlgorithms.html) in the [PatientLevelPrediction](https://ohdsi.github.io/PatientLevelPrediction/) package. In this example, to keep things simple, we select the gradient boosting algorithm.
...
The option "**Should only the first exposure per subject be included?**" is only useful if our target cohort  contains patients multiple times with different cohort start dates. In this situation, picking "yes"will result in only keeping the earliest target cohort date per patient in the analysis. Otherwise a patient can be in the data set multiple times. 

Setting "**Remove patients who have observed the outcome prior to cohort entry?**" to "Yes" will remove patients who have the outcome prior to the time-at-risk start date, so the model is fro patients who have never experience the outcome before. If "No" is selected, then patients could have had the outcome prior. Often, having the outcome prior is very predictive of having the outcome during the time-at-risk.
...
Exploring the performance of a model prediction model is easiest with the `viewPlp` function. This requires a results object as the input. If developing models in R you can use the result of `runPLp` as the input. If using the ATLAS-generated study package, then you need to load one of the models (in this example we will load Analysis_1):
...
The Shiny launches with a summary of the performance metrics on the test and train sets, see Figure \@ref(fig:shinySummary). The results show that the AUC on the train set was 0.78 and this dropped to 0.74 on the test set. The test set AUC is the more accurate measure. Overall, the model appears to be able to discriminate those who will develop AMI in new users of ACE inhibitors but it slightly over fit as the performance on the train set is higher than the test set. The ROC plot is presented in Figure \@ref(fig:shinyROC).
...
<div class="figure" style="text-align: center">
<img src="images/PatientLevelPrediction/shiny/singleShiny/singleShinyRoc.png" alt="The ROC plot for predicting AMI within a year in new users of ACE inhibitors" width="100%" />
<p class="caption">(\#fig:shinyROC)The ROC plot for predicting AMI within a year in new users of ACE inhibitors</p>
</div>
...
The table below displays the name, value (coefficient if using a general linear model, or variable importance otherwise) for all the candidate covariates, outcome mean (the mean value for those who have the outcome) and non-outcome mean (the mean value for those who do not have the outcome).
