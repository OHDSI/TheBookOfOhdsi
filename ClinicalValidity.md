...
1. Create an extremely specific (“**xSpec**”) cohort: Determine a set of persons with a very high likelihood of having the outcome of interest to be used as noisy positive cases for training a diagnostic predictive model.
2. Create an extremely sensitive (“**xSens**”) cohort: Determine a set of persons that should include anyone who could possible have the outcome. This cohort will be used to identify its inverse: the set of people we are certain of do not have the outcome.
3. Fit a predictive model using the xSpec and xSens cohort: As described in Chapter \@ref(PatientLevelPredicition), we fit a model using a wide array of patient features as predictors, and aim to predict whether a person belongs to the xSpec cohort (those we are certain have the outcome) or the inverse of the xSens cohort (those we are certain do not have the outcome).
4. Apply the fitted model to estimate the probability of the outcome for each person in a cohort: The set of predictors from the model can be applied to a person’s data to estimate the predicted probability that the person belongs to the phenotype. We use these predictions as a **probabilistic gold standard**. 
...
{\centering \includegraphics[width=0.75\linewidth]{images/ClinicalValidity/figure3} 
...
{\centering \includegraphics[width=1\linewidth]{images/ClinicalValidity/figure4} 
...
{\centering \includegraphics[width=0.75\linewidth]{images/ClinicalValidity/figure5} 
