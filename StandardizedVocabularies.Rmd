...
Each Concept is assigned a Concept ID to be used as the Primary Key. This meaningless integer ID, rather than the original code from the vocabulary, is used to record data in the CDM Event tables.  \index{concept!identifier}
...
:---------------------- | :----------------------
4000504 "Urethra part" | 36713433 "Partial duplication of urethra part"
...
Commonly used coding schemes such as ICD9 or ICD10 define more or less well-defined diagnoses based on a proper diagnostic work-up. The Condition domain is not identical with this semantic space, but partially overlapping. On one the hand, Conditions contain signs and symptoms that are recorded before a diagnosis is derived, but on the other hand, ICDs contain Concepts of different Domains (see above).
...
- Concepts within a Domain are organized into hierarchies. The quality of the hierarchy differes between Domains, and the completion of the hierarchy system is an ongoing task
