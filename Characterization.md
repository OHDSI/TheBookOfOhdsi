...
Incidence rates and proportions are statistics that are used in public health to assess the occurrence of a new outcome in a population during a time-at-risk. Figure \@ref(fig:incidenceTimeline) aims to show the components of an incidence calculation for a single person: \index{incidence}
...
\caption{Person-level view of incidence calculation components}(\#fig:incidenceTimeline)
\end{figure}
