...
>>>>>>> 60090ff8fb0f74b1dd396fc717ba667fcd9ca880
```
...
The feasibility stage does not have a well-defined process but rather is driven by various supporting activities, including identifying relevant newtork sites that contain the targeted patient population with required drug exposure, procedure information, condition or demographics information. A study lead may have enough data to do this inside their own organization and may opt for support from other OHDSI network sites. This can include asking collaborators to execute data characterization activities such as JSON files of cohort definitions from ATLAS and provisionally test of study R packages. 
...
There are multiple best practices that study teams should be following while executing the network study, including:
...
- Ensure that each network site is CDM compliant and regularly updates the OMOP Standardized Vocabularies.
- Ensure with each network site has performed and documented Data Quality checks on their OMOP CDM (e.g. ensuring ETL has followed THEMIS business rules and conventions, correct data was placed into correct CDM tables and fields).
- Ensure that each data analyst updates their local R packages to the latest OHDSI package versions before executing the study
=======
# OHDSI Network Research {#NetworkResearch}
...
- Ensure with each network site has performed and documented Data Quality checks on their OMOP CDM (e.g. ensuring ETL has followed THEMIS business rules and conventions, correct data was placed into correct CDM tables and fields).
- Ensure that each data analyst updates their local R packages to the latest OHDSI package versions before executing the study
>>>>>>> aa6d8813e3c5f351e940d416478e8d1b325cfdcf
- **Do not** tweak the study code to get desired results. If anomalies are observed, utilize Achilles or other Data Quality tools to interrogate the integrity of a contributing database. Document any findings in your manuscript discussion section.