...
We also need to obtain the **outcome status** (also referred to as the "labels" or "classes") of all the patiens during the time-at-rik. If the outcome occurs within the time-at-risk, the outcome status is defined as "positive". \index{outcome status} \index{labels} \index{classes}
...
```{r figuretheoryroctheory, fig.cap='How the ROC plots are linked to discrimination. If the two classes have similar distributions of prediced risk, the ROC will be close to the diagonal, with AUC close to 0.5.',echo=FALSE, out.width='100%'}
...
```{exercise, instantiate the cohorts}
Create a cohort table and execute the queries against the CDM to instantiate the two cohorts. How many people are in these cohort?

```

```{exercise, extract the data}
Define the covariates you want to use for the prediction and extract the plp data from the CDM. Create the summary of the plp data.
```

```{exercise, specify the population settings}
...
```{exercise, buildin and evaluating a prediction model}
